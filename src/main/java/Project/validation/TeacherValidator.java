...
public class TeacherValidator {
    ...
    public static boolean validateFTE(double fte) {
        if (fte <= 0 || fte > 1.0) {
            throw new ValidationException("Invalid FTE: Must be between 0.1 and 1.0.");
        }
        return false;
    }
    ...
    public static boolean validatePositionOrDegree(String text) {
        if (text == null || text.isBlank() || !text.matches("^[a-zA-Zа-яА-ЯіїєґІЇЄҐ'\\s]+$")) {
            throw new ValidationException("Invalid position or degree: Only letters and spaces allowed.");
        }
        return false;
    }
    ...
    public static boolean validateHireDate(String hireDate) {
        if (hireDate == null || !hireDate.matches("\\d{4}\\.\\d{2}\\.\\d{2}")) {
            throw new ValidationException("Invalid hire date: Expected format YYYY.MM.DD (e.g., 2024.05.15).");
        }
        return false;
    }
